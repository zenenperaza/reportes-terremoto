...
<div class="table-wrap"><table><thead><tr><th>C&oacute;digo</th><th>Indicador</th><th>Unidad</th><th>Coordinaci&oacute;n</th><th>Poblaci&oacute;n</th><th>Proyectos</th><th></th></tr></thead><tbody>
@foreach($indicadores as $indicador)<tr><td><strong>{{ $indicador->codigo }}</strong></td><td class="catalog-description">{{ $indicador->descripcion }}</td><td>{{ $indicador->unidad_conteo }}</td><td><span class="catalog-tag">{{ $indicador->espacio_coordinacion }}</span></td><td><span class="catalog-tag">{{ $indicador->poblacion_dirigida }}</span></td><td>{{ $indicador->proyectos_count }}</td><td class="row-actions"><a href="{{ route('indicadores.edit',$indicador) }}">Editar</a>@if($indicador->proyectos_count===0)<form action="{{ route('indicadores.destroy',$indicador) }}" method="post" onsubmit="return confirm('&iquest;Eliminar este indicador?');">@csrf @method('DELETE')<button class="danger-link" type="submit">Eliminar</button></form>@endif</td></tr>@endforeach
