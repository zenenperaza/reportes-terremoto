...
@section('title', 'Registro #'.$report->id.' | SIA')
...
<div class="details-grid">
    ...
    <section class="content-card"><h2>Actividad</h2><dl class="detail-list"><div><dt>Proyecto</dt><dd>{{ $report->proyecto?->codigo ?? $report->sector?->name }}</dd></div><div><dt>Indicador</dt><dd>{{ $report->indicadorProyecto?->indicador?->descripcion ?? $report->activity?->title }}</dd></div><div><dt>Actividad a reportar</dt><dd>{{ $report->actividadIndicador?->actividad?->descripcion ?? 'Sin actividad seleccionada' }}</dd></div><div><dt>Servicios</dt><dd>{{ $report->serviciosActividad->isEmpty() ? 'Sin servicios asociados' : $report->serviciosActividad->pluck('servicio.nombre')->join(', ') }}</dd></div><div><dt>Detalles</dt><dd>{{ $report->activity_details ?: 'Sin detalles adicionales.' }}</dd></div><div><dt>Recurrencia</dt><dd>{{ $report->recurrence_status === 'mixto' ? 'Mixta' : ($report->recurrence_status === 'recurrente' ? 'Todos recurrentes' : 'Todos no recurrentes') }}</dd></div></dl></section>
